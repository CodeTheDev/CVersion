...
public class CVersion extends JavaPlugin implements CommandExecutor, TabCompleter {
	...
	private final List<String> subcommands = Arrays.asList("help", "reload", "set");
	private final List<String> setSubcommands = Arrays.asList("version", "protocol");
	...
	public void onEnable() {
		...
		ProtocolLibrary.getProtocolManager().addPacketListener(new PacketAdapter(this, ListenerPriority.NORMAL, Collections.singletonList(PacketType.Status.Server.SERVER_INFO), ListenerOptions.ASYNC) {
			@Override
		...
		if (args.length == 0) {
			...
			if (args[0].equalsIgnoreCase("help")) {
				...
				if (args.length == 1) {
					...
					if (sender.hasPermission("cversion.set.version")) {
						...
						for (int i = 2; i < args.length; i++) {
							versionBuilder.append(args[i]).append(" ");
						}
					...
					if (sender.hasPermission("cversion.set.protocol")) {
